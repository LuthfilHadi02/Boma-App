<x-app-layout>
    <div class="flex min-h-screen bg-gray-100">
        ...
        <main class="flex-1 p-6 md:p-10">
            ...
            {{-- 🔴 NOTIFIKASI GAGAL (ANTI CRASH DATABASE) --}}
            @if(session('error'))
                <div class="p-4 mb-6 text-sm text-red-800 bg-red-50 border border-red-200 rounded-lg shadow-sm flex items-center">
                    <span class="mr-2 text-base">⚠️</span> {{ session('error') }}
                </div>
            @endif

            {{-- GRID DAFTAR CARD LAPANGAN --}}
                    ...
                    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition flex flex-col justify-between">
                        ...
                        <div class="px-5 py-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center gap-2">
                            ...
                            <div class="flex gap-1.5">
                                ...
                                {{-- FORM HAPUS (FIX rute mengarah ke mitra.facilities.destroy) --}}
                                <form action="{{ route('mitra.facilities.destroy', $item->id) }}" method="POST"
